# Proyecto-final-3 Documentación Externa 
...
# se importa la libreria tkinter que nos ayuda a crear interfaces graficas. 

class Ventana(Frame):  #Clase para crear una ventana de bloc de notas con funcionalidades básicas.
	def __init__(self, master): # nicializa la ventana principal del bloc de notas.
	...
	def widgets(self): #Crea los widgets para la interfaz gráfica de usuario del bloc de notas por lo tanto se crea las opciones.
		...
		archivo.add_command(label="Guardar como", command = self.guardar_archivo)
		...
		edicion.add_command(label="Rehacer", command = lambda: self.texto.edit_redo())
		...
		ayuda.add_separator()		
		ayuda.add_command(label="Manual del Usuario", command= self.acerca_de)
		ayuda.add_command(label="Información", command= self.informacion_de)
	...
	def ajustes_de_linea(self):  #Activa o desactiva el ajuste de línea en el texto.
	...
	def zoom_mas(self): # Aumentar el texto de la ventana
		if self.n <90:			
			self.n += 2
	...
	def zoom_menos(self): #disminuye el texto de la ventana 
		if self.n >6:			
			self.n -= 2
			self.texto.config(font= (self.f, self.n))
		else:
	...
	def informacion_de(self):
	    ...
	    vent_info.resizable(0,0)
	    vent_info.iconbitmap('icono.ico')
	    vent_info.geometry('460x350+350+350')
	    Label(vent_info, bg='white', 
		text= 'Aplicación acerca de un bloc de notas dinámico en el cual se puede escribir \n y guardar notas, además de tener una \n experiencia diferente al convencialnal bloc de notas \n\n Con la licencia de: Software libre \n\n versión: 1.0').pack(expand=True)	
	...
	def elegir_color_texto(self): # elige el color del texto
		color = colorchooser.askcolor()[1] 
		self.texto.config(fg= color, insertbackground = color)

	def elegir_color_fondo(self): # elige el color del fondo del block de notas
		color = colorchooser.askcolor()[1]
		self.texto.config(bg= color)


if __name__ == "__main__": #punto de entrada 
	ventana = Tk() #inicializa la ventana la ventana principal
	app = Ventana(ventana)  # crea clase ventana
	app.mainloop() # incia el bucle principal 
